import type { DeadlineCampaignScheduleMode } from "../types/api";
...
export function describeSendSchedule(params: {
  ...
  if (slots.length === 0) return "Multiple times per day";
  return `${slots.length} times per day at ${slots.join(", ")} (UK), every ${params.sendIntervalHours} hour(s) from ${normalizeHm(params.sendStartTime)}`;
}
