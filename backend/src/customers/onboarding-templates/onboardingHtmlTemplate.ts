...
/** One table row per director with any data; empty list renders a single blank row for layout. */
function buildDirectorTableRowsHtml(directors: CustomerOnboardingData["directors"]): string {
  const meaningful = directors.filter(
    (d) =>
      d.name.trim() ||
      d.address.trim() ||
      d.city.trim() ||
      ...
      d.personal_utr.trim() ||
      d.ni_number.trim() ||
      d.date_of_birth.trim() ||
      d.identity_verification_code.trim(),
  );
  const rows: DirectorRowFields[] =
    meaningful.length > 0
      ? meaningful
      : [
          {
...
export type RegistrationHtmlOptions = {
  /**
   * Page origin (e.g. https://app.example.com). Used when {@link logoSrc} is not set and the
   * bundled logo asset is unavailable.
   */
  assetOrigin?: string;
...
function brandLogoAbsoluteSrc(opts?: RegistrationHtmlOptions): string {
  ...
  const embedded = getBrandLogoDataUrl();
...
export function renderRegistrationHtml(
  ...
  const signatureBlockHtml = sigTrim.startsWith("data:image/")
    ? `<div style="margin-top:10px"><span class="sig-label">Signature</span><br/>${signatureForPdfHtml(
        sigTrim,
        "max-height:56px;max-width:220px;object-fit:contain;vertical-align:middle",
      )}</div>`
...
${tradingAddressSectionHtml}
  ...
  <table class="form-tbl" style="margin-top:6px">
    <tbody>
      <tr>
        <td class="lbl" style="width:22%; vertical-align:top">Notes :</td>
        <td class="val" colspan="3" style="white-space:pre-wrap; vertical-align:top; min-height:2.2em">${v(m.officeNotes)}</td>
      </tr>
      <tr>
        <td class="lbl">Internal remarks :</td>
        <td class="val" colspan="3" style="white-space:pre-wrap; vertical-align:top; min-height:2.2em">${v(m.officeInternalRemarks)}</td>
      </tr>
      <tr>
  ...
  <table class="form-tbl" style="margin-top:-1px">
    <tbody>
      <tr>
